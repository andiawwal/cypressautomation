...
describe('Users - Create New',function () {
    ...
    describe('Create New UI Verification',function () {
        ...
        it('S-Voucher-98:Verify Click Create New button', function () {
            cy.url().should('eq','http://asdasd.com/administration/users/new')
        });

        it('S-Voucher-99:Verify Create New User UI',function () {
        ...
        describe('Empty Fields Verification',function () {
            ...
            it('S-Voucher-100:Verify Submit with all field are empty', function () {
                fieldVerification([0,1,2,3])
            });

            it('S-Voucher-101:Verify Submit with Username\'s field empty',function () {
                fieldInput([1,2,3],dummyData)
                fieldVerification([0])
            })

            it('S-Voucher-102:Verify Submit with Email field empty',function () {
                fieldInput([0,2,3],dummyData)
                fieldVerification([1])
            })

            it('S-Voucher-103:Verify Submit with Password\'s field empty',function () {
                fieldInput([0,1,3],dummyData)
                fieldVerification([2])
            })

            it('S-Voucher-104:Verify Submit with role field isn\'t selected',function () {
    ...
    describe('Cancel',function () {
        it('S-Voucher-106:Verify Cancel with all field empty', function () {
        ...
        it('S-Voucher-107:Verify cancel with all field already filled', function () {
            cy.get('tr[class=ng-scope]').eq(0).then($el =>{
                cy.get('.btn-primary').contains('Create new').click()
                    .wait(3000)
